...
const exec_name = process.argv.length > 3 ? process.argv[3] : 'strapi-app';
...
if (destSubFolder !== 'bundled') {
  ...
  glob('packages/server/bundled/*.node')
    .then((files) => {
      if (files.length === 0) {
        console.error('No node files found, but there should be a node file for sharp, did you add node-loader to webpack config?');
        return;
      }
...
//const libvipsPattern = path.join('node_modules', '@img', `sharp-libvips-${platformArch}`, 'lib', `-cpp.${extension}`);
const libvipsPattern = 'node_modules/**/sharp/vendor/**/libvips*';
const libvipsPattern2 = path.join('node_modules', '@img', `sharp-libvips-${platformArch}`, 'lib', 'libvips*');
console.log(`Searching for libvips with pattern: ${libvipsPattern}`);

glob(libvipsPattern)
  .then(async (files:any) => {
    if (files.length === 0) {
      console.log(`Searching for libvips with pattern: ${libvipsPattern2}`);
      files = await glob(libvipsPattern2).then((files) => {
          if (files.length === 0) {
            console.error('No matching libvips file found');
            return;
          }
          return files;
        });
    ...
    if (files){
      ...
      console.log(`Copying ${sourceFile} to ${targetFile}`);
      fs.copyFileSync(sourceFile, targetFile);
      console.log('libvips copied successfully');
    }
  })
  .catch((err) => {
